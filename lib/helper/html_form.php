...
class HtmlFormHelper extends Helper {
	 ...
	 */
	public function iText($key, $autocomplete=true) {
	 ...
	 */
	public function iSelect($key, $options='', $trSection='') {
		if (isset($this->obj)) {
			$options = $this->obj->getPropertyOptions($key);
		}
		if (empty($options['options'])) {
			FC::log_warn("iSelect $key does not provide options");
			return $key.' (no option)';
		}
		...
		foreach($options['options'] as $val => $lbl) {
			...
			$str .= ' value="'.$val.'">'.($trSection ? TR::html($trSection,$lbl) : $lbl).'</option>';
			$i++;
		}
		return $str.'</select>';
	}
	...
	/**
	 * generate select from list object
	 */
	public function iSelectList($key, $value, $nooption='', $extra='') {
		$str = '<select name="'.$key.'"';
		if ($extra) {
			$str .= ' '.$extra;
		}
		$str .= '>';
		if ($nooption) {
			$str .= '<option value="">'.$nooption.'</option>';
		}
		while ($this->obj->next()) {
			$uid = $this->obj->getUid();
			$str .= '<option value="'.$uid.'"';
			if ($value == $uid) {
				$str .= ' selected="selected"';
			}
			$str .= '>'.$this->obj->getLabel().'</option>';
		}
		$str .= '</select>';
		$this->obj->rewind();
		return $str;
	...
	/**
	 * generates a select (drop down) from list
	 */
	public function iSelectDb($key, $name, $objlist, $default='', $nooption='') {
		if (empty($default)) {
			$default = $this->obj->value($key);
		}
		$str = '<select id="i_'.$key.'" name="'.$key.'">';
		if ($nooption) {
			$str .= '<option value="">'.$nooption.'</option>';
