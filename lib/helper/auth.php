...
class AuthHelper extends Helper {
	...
	public function checkPassword($password) {
        ...
        var_dump(crypt($password, $salt)==$pass);
	...
	*/
    ...
    public function updateSessionVariables() {
    	...
    	$_SESSION["appNickName"] = $this->obj->get('nickname');
		$_SESSION["appUserTimeZone"] = $this->obj->get('time_zone');
		$_SESSION["appUserLang"] = $this->obj->get('language');
    ...
    public function setAutoLogin() {
        if (($this->obj->getUid()) && ($this->obj->get('salt'))) {
            setCookie('auto_login',
            	$this->obj->getUid().":".$this->obj->get('salt'),
            	time()+(3600*24*30), APP_WWW_URI, $_SERVER['SERVER_NAME'],
	...
	*/
    public function resetAutoLogin() {
        if (($this->obj->getUid()) && ($this->obj->get('salt'))) {
            setCookie('auto_login', '',
            	APP_WWW_URI, $_SERVER['SERVER_NAME'],
            	isset($_SERVER['HTTPS'])?true:false
            );
	...
	*/
    public function isLoggedIn($load=false) {
        ...
        if ($_SESSION['appUserId'] == 0 || $this->isLoggingOut) {
			...
			$this->obj->set('nickname', $_SESSION['appNickName']);
	        ...
	        // check user IP
	        if (isset($_SESSION['appUserCurrentAddress']) && $_SERVER['REMOTE_ADDR'] != $_SESSION['appUserCurrentAddress']) {
