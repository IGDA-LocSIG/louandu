 ...
 */
class DbHelper extends Helper implements Callabled {
	...
	public function next() {
		...
		if (array_key_exists($this->idx, $this->rows)) {
			...
			$this->obj->resetProperties(true);
			return false;
		}
	}
	
	public function cloneData() {
		$cl = get_class($this->obj);
		$obj = new $cl();
		$i = $this->idx - 1;
		$obj->set($this->rows[$i]);
		return $obj;
	...
	public function pagination($url, $param='pg', $lmt=7) {
		...
		$max = 0;
		while (($max * $this->size) < $this->total) {
			$max++;
		}
		...
		if ($start > 1) {
			$str .= '<li><a href="'.$url.'/'.$param.',1">&laquo;</a></li>';
		}
		
		if ($this->page > 1) {
			$str .= '<li><a href="'.$url.'/'.$param.','.($this->page - 1).'">&lsaquo;</a></li>';
		} else {
			$str .= '<li class="disabled"><a>&lsaquo;</a></li>';
		}
		
		for ($i=$start; $i<=$stop; $i++) {
			if ($this->page == $i) {
				$str .='<li class="active">'
					.'<a href="'.$url.'/'.$param.','.$i.'">'.$i.'</a>'
					.'</li>';
			} else {
				$str .= '<li><a href="'.$url.'/'.$param.','.$i.'">'.$i.'</a></li>';
			}
		}
		
		if ($this->page < $stop) {
			$str .= '<li><a href="'.$url.'/'.$param.','.($this->page + 1).'">&rsaquo;</a></li>';
		} else {
			$str .= '<li class="disabled"><a>&rsaquo;</a></li>';
		}
		
		if ($stop < $max) {
			$str .= '<li><a href="'.$url.'/'.$param.','.$max.'">&raquo;</a></li>';
		}
