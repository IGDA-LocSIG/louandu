...
class AuthAclHelper extends AuthHelper {
	...
	* check ACL
	*/
	public function checkAcl($code, $and=false) {
		if (!is_array($this->_arrAcl)) {
			$this->_arrAcl = explode(',', $this->obj->get('actags'));
		}
		$test = explode(',',$code);
		if ($and) {
			// all required rights needed
			foreach ($test as $c) {
				$c = trim($c);
				if (!in_array($c, $this->_arrAcl)) {
					return false;
				}
			}
			return true;
	...
	*/
	 ...
	 */
	public function updateACL($section='', $class='') {
		$id = $this->obj->getUid();
		if (!$id) {
			return false;
		}
		...
		while ($objLst->next()) {
			if ($fc->request->get('acl_'.$objLst->get('name')) ||
					in_array($objLst->get('name'), $GLOBALS['config']['users']['class']['acl'][$class])) {
