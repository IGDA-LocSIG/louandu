...
abstract class Model extends Pluginable {
    ...
    public function initObjectProperties($nested = false) {
		foreach($this->_properties as $key => $type) {
			if (preg_match('/^OBJ/i',$type)) {
				$class = ((strlen($type) > 3)?substr($type,4):$key).'Model';
				if ($nested) {
					...
					$this->data[$key]->initObjectProperties(true);
				}
			}
		}
	...
	public function resetProperties($nested=false) {
		foreach($this->_properties as $key => $type) {
			if (preg_match('/^OBJ/i',$type)) {
				$class = ((strlen($type) > 3)?substr($type,4):ucfirst($key)).'Model';
				$this->data[$key] = new $class();
				if (!$nested) {
					$this->data[$key]->resetProperties(true);
				}
			} else {
				unset($this->data[$key]);
			}
	...
	public function set()
	{
		...
		if (is_array($data) || is_object($data)) {
			// first parameter is an array or object ? Setting all (selected) properties then !
			$ok = true;
			foreach($this->_fields as $key) {
				if ($this->isObjectProperty($key, $class, $nkey)) {
					...
					if (is_object($data)) {
						$val = (property_exists($data, $key)) ? $data->$key : NULL;
					}
					// set data
	...
	public function htmlError($key) {
		if ($str = $this->getError($key)) {
			return '<div class="alert error">'.VarStr::html($str).'</div>';
		} else {
			return false;
		}
	}
...
class VarDec extends VarAbstract
{
	public static function sanitize($val, &$info) {
		...
		if (preg_match('/[0-9]*(\.[0-9]+)?/',$val)) {
			// if there's a printf option...
			foreach ($info as $option) {
				if (preg_match('/^%/', $option)) {
					$val = preg_replace('/,/','.',sprintf($option,$val));
				}
			}
			return $val;
		} else {
	...
	public static function html($val, $dec=null, $default='') {
		if (is_null($dec)) {
			return $val;
		} else {
...
class VarDte extends VarAbstract
{
		...
		if (!$GLOBALS['config']['datetime']['us_format']) {
			...
			if (preg_match('/^([0-3]?[0-9])\/([0-1][0-9])(\/(20)?[0-9]{2})?$/',$val, $arr)) {
				...
				if (empty($arr[3])) {
					...
					$arr[3] = substr(APP_YEAR,0,2).$arr[3];
				}
	...
	public static function unixToStr($val, $format=APP_DATE_SQL) {
...
{
	public static function sanitize($val, &$info) {
		if (empty($val) && in_array('compulsory',$info)) {
